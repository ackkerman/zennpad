...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider("zennExplorer", treeDataProvider)
  );

  const fsProvider = new ZennFsProvider();
  context.subscriptions.push(
    vscode.workspace.registerFileSystemProvider("zenn", fsProvider, {
      isCaseSensitive: true
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("zenn.refresh", () => {
      treeDataProvider.refresh();
      vscode.window.showInformationMessage("Zenn content refreshed (scaffold)");
    }),
    vscode.commands.registerCommand("zenn.newArticle", async () => {
      const uri = vscode.Uri.parse(`zenn:/articles/${Date.now()}_draft.md`);
